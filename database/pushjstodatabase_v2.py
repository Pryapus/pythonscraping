...
def updatetodatabase(DatabaseName, NameFirstRow, DataFirstRow, NameSecondRow, DataSecondRow):
    data = supabase.table(DatabaseName).update({NameFirstRow: DataFirstRow, NameSecondRow: DataSecondRow}).execute()
    assert len(data.data) > 0
